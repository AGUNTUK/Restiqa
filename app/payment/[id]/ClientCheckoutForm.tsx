...
export default function ClientCheckoutForm({ bookingId, amount, dict }: ClientCheckoutFormProps) {
  ...
  const [method, setMethod] = useState("zinipay");
  ...
  const handleZiniPay = async () => {
    ...
    try {
      const response = await fetch("/api/payment/zinipay/create", {
        method: "POST",
      ...
      if (!response.ok) {
        throw new Error(data.error || "Failed to initiate ZiniPay payment");
      }
      ...
      console.error("ZiniPay initiation error:", err);
  ...
  return (
    <form onSubmit={handleCheckout} className="w-full max-w-lg mx-auto">
      ...
      {/* Custom Payment UI Section - ONLY for manual methods */}
      <div className={method === "zinipay" ? "hidden" : "block"}>
      ...
      <div className={method === "zinipay" ? "block" : "hidden"}>
        <div className="bg-[#f0f3f8] p-8 rounded-[32px] mb-8 shadow-[inset_4px_4px_8px_#c4c9ce,inset_-4px_-4px_8px_#ffffff] text-center space-y-4">
          <div className="w-16 h-16 bg-[#d32f2f]/10 rounded-2xl flex items-center justify-center mx-auto mb-2 text-[#d32f2f]">
            <svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="m13 2-2 10h3L11 22l2-10h-3L13 2z"/></svg>
          </div>
          <h4 className="text-xl font-extrabold text-[#1a202c]">Automatic Gateway</h4>
          <p className="text-[#718096] text-sm leading-relaxed px-4">
            Pay instantly via <strong>bKash, Nagad, or Rocket</strong>. Your booking will be confirmed automatically upon completion.
          </p>
        </div>
      </div>
      ...
      {error && (
        ...
        <button 
          type="submit" 
          disabled={loading || (method !== "zinipay" && (!transactionId.trim() || !amountPaid || !senderNumber.trim() || !transactionTime))}
          className={`w-full py-4 px-6 rounded-2xl font-bold text-lg transition-all duration-300 flex items-center justify-center gap-3 ${
            loading || (method !== "zinipay" && (!transactionId.trim() || !amountPaid || !senderNumber.trim() || !transactionTime))
        ...
        {loading ? (
          <>
            ...
            <span>{method === "zinipay" ? "Redirecting..." : "Verifying..."}</span>
          </>
        ) : (
          <span>{method === "zinipay" ? "Pay Now" : "Confirm Payment"}</span>
        )}
      </button>
    </form>
  );
